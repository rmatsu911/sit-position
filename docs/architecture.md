...
画面ごとに同じ計算を書き直さないよう、次の2つに集約している。
どちらも純粋な関数で、React にも API にも依存しない（テストから直接呼べる）。

| 場所 | 責務 | テスト |
| --- | --- | --- |
| `src/lib/timeline.ts` | 日時 → 座標（Asia/Tokyo 固定・`[開始, 終了)` の半開区間・3時間〜年の表示単位・ドラッグの刻み） | `npm run test:timeline` |
| `src/lib/cpm.ts` | クリティカルパス（ES/EF/LS/LF/total float） | `npm run test:cpm` |

**クリティカルパスは工程名では決めない。** 期間と先行工程（`task_dependencies`）だけで
決まるため、工程名を変えても結果は変わらず、期間や依存を変えれば結果が変わる。
扱いを決めてあるもの:

- 日程未設定の工程は所要時間が決まらないため計算せず、クリティカルにもしない
- 先行も後続も無い孤立工程は計算対象に含める（単独で最長ならクリティカル）
- 循環している依存はその閉路を計算対象から外し、画面に「循環」と示す
- 親工程は子の集計であり作業ではないため、経路には載せない
- 工程は「予定開始より前」にも「先行工程が終わる前」にも始められない
