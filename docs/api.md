...
| メソッド | パス | 内容 |
| --- | --- | --- |
| GET | `/projects/{id}/audit-logs?limit=` | この案件の操作履歴（認証＋案件スコープ） |

`audit_logs`（誰が何を操作したか）と `task_change_history`（工程のどの項目が
どう変わったか）を1本の時系列にまとめて返す。記録が無ければ空配列で、
画面は「まだありません」と表示する（固定の履歴文言は持たない）。
表示文はサーバー側で組み立てるため、画面ごとに文言がぶれない。
利用者が紐づかない操作は「利用者不明」と返す（実際にはシステムが行っていない
操作を「システム」とは書かない）。
