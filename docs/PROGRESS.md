...
1. **Phase 4 のリリース確認**：Render の参照ブランチ設定、実環境のリビジョン一致、
   migration の適用状況（`docs/deploy-render.md` 参照）
2. **図面提出期限**：`documents` に期限日の列が無く、カレンダーへ統合できていない。
   列の新設（migration）と運用の決定が必要
3. **Phase 5以降**：個人設定（表示設定・職種と権限の分離・カレンダー購読）、報告強化、現場連絡、AI高度化
...
- 2026-08-03 Ver.0.4 Phase 4 完了（案件ライフサイクルと工程への業務導線：`GET /projects/search` と `/projects/filter-options` を新設し、キーワード・状態・現場責任者・担当会社・エリア・担当部署・遅延有無・予定期間の全条件をサーバー側でANDで適用。担当会社は案件ではなく**工程の担当会社**を EXISTS で判定し、同じ会社の工程が複数あっても案件が重複しない。`total` は権限・案件スコープと全条件を適用したあと・ページネーションより前に数え、全ページの合計と一致する。絞り込みの選択肢は表示中のページではなく権限範囲の全案件から生成。静的ルートを `/{project_id}` より前に定義してルート衝突を防止。登録・更新は工事番号の重複を409、`IntegrityError` も409、開始日>終了日を422（更新は保存後の値の組で判定）。既定の並び順は `created_at DESC, id DESC` で登録した案件が必ず1ページ目の先頭に出る。条件で隠れる場合は「登録済みだが非表示」と案内し条件解除の導線を用意。案件一覧の検索条件はURLを正とし、URL⇔APIパラメータ⇔React Query の queryKey を1対1に統一。施工写真・品質管理・現場日報・報告書の固定案件参照を撤去し、共通の案件セレクタ（未選択が既定・URLへ保存）へ移行。報告書は選択案件の工程を下書きとし固定の工事名・作成日・作成者を撤去。実ブラウザ回帰で右クリックメニューが開かない不具合（メニュー自身の閉じる購読が開いた直後に発火）と、更新権限が無いロールに編集ボタンが出る問題を検出・修正。DB変更・migration なし。timeline113/pytest116/lint0/build/18ルート回帰0/座標実測25/完了確認39/表示単位50/マイルストーン画面127/出力一致130/統合検証71/表示名監査0件/カレンダー61/Phase4回帰92 すべて一致）
- 2026-08-03 Ver.0.4 Phase 4 追補（案件切替時のデータ分離と固定表示の是正）：現場日報・施工写真で、案件を切り替えても前の案件の状態（`currentId`／`draft`／`photos`／選択・ライトボックス・分類対象・反映済み）が残る問題を修正。`useEffect` では1回描画されてしまうため、案件IDが変わったらレンダリング前に初期化する方式へ統一し、読み込み中・未選択でも前の案件を描画しない。施工写真・品質管理・報告書は未選択時に案件セレクタと空状態だけを出し、一覧・アップロード・一括操作・試験記録追加・AI品質チェック・行編集・出力・印刷をいずれも描画しない。`projectId ?? 0` と `projectId as number` を撤去し、写真アップロードと試験記録は登録先の案件を実行前に検証する（APIも 404／403 で拒否することをテストで固定）。品質管理の `q.projectId === 'p1' ? '熊本中央局' : …` という固定案件名判定、報告書プレビューの固定工事名、施工写真の詳細に残っていた固定案件名、案件詳細「概要」「作業内容」タブの固定文言を撤去し、いずれも選択案件の実データ（工程・資材・未確認写真・品質確認）から表示する。`/schedule` が先頭案件を自動選択していた挙動も撤去。実ファイルを生成しない出力に出ていた「デモ環境のため、実ファイルは生成されません」という進捗演出（StepRunner）を廃止し、未対応であることと実出力に対応している帳票を明示。未参照の固定データ `src/data/projects.ts` を削除。固定案件参照の監査を `scripts/fixed-project-audit.mjs` としてゲート化（残存0件）。pytest118／timeline113／lint0／build／18ルート回帰0／座標実測25／完了確認39／表示単位50／マイルストーン画面127／出力一致130／統合検証71／表示名監査0件／カレンダー61／固定案件参照0件／Phase4回帰132 すべて一致）
- 2026-08-05 Ver.0.5 Phase 5 完了（実環境整合性・帳票・案件コンテキスト・全画面回帰）：**帳票と工程データの不一致を修正**（PDF/Excel が「WBSにドットがある工程」だけを対象にしており、親子を作っていない案件では0行になっていた。`parent_task_id` と WBS 接頭辞の両方から末端工程を判定する `leaf_tasks()` へ統一し、`GET /reports/{type}/preview` を新設して画面・PDF・Excel・CSV が同じ `ReportSpec` を共有するようにした）。**案件コンテキストを一元化**（`useSelectedProject()` が「パスの `:id` → `?project_id=`」の順で解決する唯一の入口になり、固定案件名・`DEMO_PROJECT_ID`・`projectId ?? 0`・`projectId as number` を全撤去。案件配下ルートを7本新設し、11画面で同じ案件名・工事番号が出る。案件切替は `ProjectScope` ＋ `flushSync` でURL変更前に内容を伏せ、`key={projectId}` で state 境界を作ることで、前の案件が**1フレームも残らない**ことを除外条件なしで実測）。**マイルストーン登録を修正**（案件候補を既存マイルストーンからではなく権限範囲の案件から作る）。**工程の時間精度・複数先行工程・工程フォーム**（日／半日／時間の3粒度、ドラッグの刻みを粒度に合わせ[時間単位は1時間]、先行工程を複数選択・自己依存と循環を422で拒否・変更を `task_change_history` へ記録、親工程／工種／工程種別／担当班／責任者／担当会社／遅延理由／変更理由を追加し選択肢は `GET /projects/{id}/task-form-options` の実データから、`tasks.team_id` を追加[migration e7c4b2f10a93]、終了が開始以前になる保存を422で拒否[リサイズで作れていた]、編集で備考が消え予定進捗が実績で上書きされる不具合を修正）。**クリティカルパスを工程名から計算へ**（`CRITICAL_NAMES` の12個の工程名一致を撤去し、`src/lib/cpm.ts` で ES/EF/LS/LF/total float を計算。日程未設定・孤立工程・循環・親工程の扱いを明示し、工程名を変えても結果が変わらないことをテストで固定）。**操作履歴を実データへ**（案件詳細の固定5行を撤去し、`GET /projects/{id}/audit-logs` が `audit_logs` と `task_change_history` を1本の時系列で返す）。**システム情報とAI機能の状態を正確化**（`ai_service` の1語を内訳[登録済み/学習済みモデル・処理待ちジョブ・最後に成功した推論・Worker状態]へ分割し、待ちジョブが無いだけの状態を「稼働中」と書かない。`GET /ai/status` が7機能それぞれを not_implemented / model_missing / service_down / failed / processing / connected で返し、画面は状態を持たない。`api_root_path` とベースパスを並べて表示）。**全17画面の実データ監査**（図面の未選択時に「図面が登録されていません」と出していた誤り、現場日報の見出し欠落、横断3画面で対象案件を示していなかった点、工程管理で実際には設定できる操作を「準備中」と表示していた点を是正）。**最小CIとまとめ実行**（`.github/workflows/ci.yml` で typecheck/lint/build/timeline/cpm/固定案件参照/pytest/migration往復、`scripts/dev-stack.sh` と `npm run verify:all` で11ゲートを1枚に集約）。検証: typecheck/ESLint 0 error/build/timeline130/cpm49/固定案件参照0件/pytest144/Phase4回帰172/Phase5工程フォーム59/Phase5状態表示53/Phase5全画面監査136 すべて 0 failed。migration は PostgreSQL で upgrade→downgrade→upgrade を実行し既存工程121件の保持を確認。**公開環境（Xserver / Render）へは未反映で、実環境のリビジョンは未確認。**
