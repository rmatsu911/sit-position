...
| **施工写真** | 5種の表示切替／タグ・確認状況フィルタ／拡大（ライトボックス）／コメント・確認・再撮影／アップロードデモ／写真分類支援（分類候補・確認候補・画像認識イメージの参考表示） |
| 図面 | 図面一覧＋ビューア（拡大/縮小/回転/ピン/コメント/版比較） |
| **品質管理** | 集計／一覧／詳細モーダル（承認・再撮影・保留）／**施工品質比較**（確認対象 vs 完成基準の写真比較・確認候補の参考表示） |
...
- 画像認識・分類は実装しておらず、施工写真の分類候補・確認候補・画像認識イメージはすべて検証用の固定データによる参考表示です（AI機能構想として将来追加予定）。
- 実在の顧客・個人情報は使用していません。
